...
int main(int argc, char **argv) {
  ...
  for (int imsg = 0; imsg < TAILLE_MSG; imsg++) {
    for(int istate = 0 ; istate < NB_STATE ; istate++) {
      if(trellis[imsg][istate].pere!=NULL) {
        for(int i = 0 ; i < 2 ; i++) {
          if(trellis[imsg+1][  diag[istate][i][STATE]   ].poids > (trellis[imsg][istate].poids + distance_hamm_bin_de_lg2(diag[istate][i][OUTPUT],msg[imsg]))  ) {
            trellis[imsg+1][  diag[istate][i][STATE]   ].pere = &trellis[imsg][istate];
            trellis[imsg+1][  diag[istate][i][STATE]   ].poids = trellis[imsg][istate].poids + distance_hamm_bin_de_lg2(diag[istate][i][OUTPUT],msg[imsg]);
            trellis[imsg+1][  diag[istate][i][STATE]   ].origin = i;
          }
        }
      }
    }
  ...
  for(int i = 1 ; i < NB_STATE ;i++)
    if(trellis[TAILLE_MSG+1][i].poids < trellis[TAILLE_MSG+1][select].poids)
      select = i;


  p_noeud courant = &trellis[TAILLE_MSG+1][select];
  int j = 0;
  while(courant->pere != (p_noeud)1) {
    msg[j++] = courant->origin;
